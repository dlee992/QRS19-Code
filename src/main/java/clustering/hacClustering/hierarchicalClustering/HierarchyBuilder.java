...
public class HierarchyBuilder {
    ...
    public void agglomerate(LinkageStrategy linkageStrategy) {
        ...
        if (minDistLink != null) {
        	//System.out.println("  MinDist In All Pairs "+minDistLink.getLinkageDistance());
    ...
    public boolean isTreeCut(double epsilon){
  	  if(distances.list2().size()!=0){
  		  List<ClusterPair> cps = new ArrayList<ClusterPair>();
  		 
  		 cps.addAll(distances.list2());
  		 Collections.sort(cps);

  		  return cps.get(0).getLinkageDistance()>epsilon;
